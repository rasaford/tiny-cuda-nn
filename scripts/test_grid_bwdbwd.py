...
class SDF(nn.Module):
    def __init__(self, hash=True, n_levels=12, log2_hashmap_size=15, base_resolution=16, smoothstep=False) -> None:
        ...
        self.decoder = nn.Sequential(nn.Linear(self.encoder.n_output_dims, 64), nn.ReLU(True), nn.Linear(64, 1))
    ...
    def forward_with_nablas(self, x):
        with torch.enable_grad():
            ...
            nablas = autograd.grad(
                sdf, x, torch.ones_like(sdf, device=x.device), create_graph=True, retain_graph=True, only_inputs=True
            )[0]
        return sdf, nablas
...
if __name__ == "__main__":
    ...
    def grad_check():
        ...
        def apply_on_x(x):
            params = model.encoder.params.to(
                _torch_precision(model.encoder.native_tcnn_module.param_precision())
            ).contiguous()
            return _module_function.apply(model.encoder.native_tcnn_module, x, params, 128.0)
        ...
        #     ✓ dL_dx   w.r.t. dL_dy    i.e. ddLdx_ddLdy    (passed) WHAT IS THIS GRADIENT?
        ...
        autograd.gradcheck(backward_apply_on_params, model.encoder.params, eps=1.0e-3)
    ...
    def vis_graph():
        ...
        sdf, nablas = model.forward_with_nablas(x)
        # runs a backward pass through the model to produce this plot.
        # I.e. you see which modules are called on a .backward() call to (sdf, nablas)
        torchviz.make_dot(
            (nablas, sdf, x, model.encoder.params, *list(model.decoder.parameters())),
            {
                ...
                "sdf": sdf,
                "x": x,
                "grid_param": model.encoder.params,
